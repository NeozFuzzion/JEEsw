...
public interface MonsterRepository extends JpaRepository<Monster, MonsterId> {
    ...
    @Query("SELECT m from Monster m WHERE m.gameMonster.natural_stars = 5L and m.jSON=:jsonChosen and m.gameMonster.obtainable = true and m.idMonster.user=:user")
    List<Monster> findAllNatural5Monsters(@Param("user") User user,@Param("jsonChosen") String jsonChosen);
    @Query("SELECT m from Monster m WHERE m.gameMonster.natural_stars = 4L and m.gameMonster.obtainable = true and m.jSON=:jsonChosen and m.idMonster.user=:user")
    List<Monster> findAllNatural4Monsters(@Param("user") User user,@Param("jsonChosen") String jsonChosen);
    @Query("SELECT m from Monster m WHERE m.gameMonster.awaken_lvl = 2 and m.gameMonster.obtainable = true and m.jSON=:jsonChosen and m.idMonster.user=:user")
