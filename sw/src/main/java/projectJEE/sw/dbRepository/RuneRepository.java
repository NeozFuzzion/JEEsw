...
public interface RuneRepository extends JpaRepository<Rune, RuneId> {
    ...
    List<Rune> findAllAncientBySet(@Param("set") RuneSet set, Sort sort);

    @Query("select r from Rune r where r.set_id=:set and r.classe < 7")
    List<Rune> findAllNonAncientBySet(@Param("set") RuneSet set,Sort sort);
    @Query("select r from Rune r where r.set_id=:set")
    List<Rune> findAllBySet(@Param("set") RuneSet set,Sort sort);
